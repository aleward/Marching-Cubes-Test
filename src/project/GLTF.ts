...
export class GLTF extends Primitive
{
  ...
  public async loadModel(url:string, centering:boolean = false):Promise<void>
  {
    ...
    for(let i:number = 0; i < this._numVertices; i++)
    {
      ...
      this._bufferData[bufferVertexOffset] = (<any>data.position.data)[sourceVertexOffset] - centerX;
      this._bufferData[bufferVertexOffset + 1] = (<any>data.position.data)[sourceVertexOffset + 1] - centerY;
      this._bufferData[bufferVertexOffset + 2] = (<any>data.position.data)[sourceVertexOffset + 2] - centerZ;

      this._bufferData[bufferVertexOffset + 3] = (<any>data.normal.data)[sourceVertexOffset];
      this._bufferData[bufferVertexOffset + 4] = (<any>data.normal.data)[sourceVertexOffset + 1];
      this._bufferData[bufferVertexOffset + 5] = (<any>data.normal.data)[sourceVertexOffset + 2];
    }
  }
